...
RDS_SECURITY_GROUP_ID=$(aws ec2 describe-security-groups \
    ...
    --output text) && echo "$RDS_SECURITY_GROUP_ID"
...
PRIVATE_SUBNET_1a_ID=$(aws ec2 create-subnet \
    ...
    --output text) && echo "$PRIVATE_SUBNET_1a_ID"
...
PRIVATE_SUBNET_1c_ID=$(aws ec2 create-subnet \
    ...
    --query "Subnet.SubnetId" \
    --output text) && echo "$PRIVATE_SUBNET_1c_ID"
...
PRIVATE_ROUTE_TABLE_ID=$(aws ec2 create-route-table \
    ...
    --output text) && echo "$PRIVATE_ROUTE_TABLE_ID"
...
INSTANCE_ID=$(aws ec2 describe-instances \
    --filter Name=tag:Name,Values=$EC2_NAME Name=instance-state-name,Values=running \
